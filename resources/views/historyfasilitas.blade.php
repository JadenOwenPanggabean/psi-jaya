...
<div class="section padding-top-bottom over-hide background-grey">
    <div class="container-fluid"> 
        <table class="table table-hover"> 
            ...
            <tbody>
                @php
                    $number = ($datahapus->currentPage() - 1) * $datahapus->perPage() + 1;
                @endphp
                @foreach ($datahapus as $index => $data)
                <tr>
                ...
                <!-- Modal -->
                <form action="{{ route('repairAvailable', $data->id) }}" method="POST">
                    @csrf
                    @method('PUT')
        ...
        <nav class="bg-white flex items-center flex-column flex-wrap md:flex-row justify-between p-4" aria-label="Table navigation">
            <span
                ...
                Showing
                <span class="font-semibold text-gray-900 dark:text-white">{{ $datahapus->firstItem() }}</span>
                to
                <span class="font-semibold text-gray-900 dark:text-white">{{ $datahapus->lastItem() }}</span> of
                <span class="font-semibold text-gray-900 dark:text-white">{{ $datahapus->total() }}</span>
            </span>

            <div class="w-full md:w-auto text-right">
                <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                    <!-- Previous Page Link -->
                    @if ($datahapus->onFirstPage())
                    ...
                    @else
                        <li>
                            <a href="{{ $datahapus->previousPageUrl() }}"
                    ...
                    <!-- Pagination Elements -->
                    @foreach ($datahapus->links()->elements[0] as $page => $url)
                        @if ($page == $datahapus->currentPage())
                    ...
                    <!-- Next Page Link -->
                    @if ($datahapus->hasMorePages())
                        <li>
                            <a href="{{ $datahapus->nextPageUrl() }}"
